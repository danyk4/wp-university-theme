...
add_action( 'rest_api_init', 'uniLikeRoute' );

function uniLikeRoute() {
...
function createLike( $data ) {
	if ( is_user_logged_in() ) {
		...
		if ( $existQuery->found_posts == 0 && get_post_type( $professor ) == 'professor' ) {
			...
			wp_die( 'Invalid professor id' );
		}
	} else {
		wp_die( 'Only logged in users can create a like.' );
	}
}
...
function deleteLike( $data ) {
	...
	if ( get_current_user_id() == get_post_field( 'post_author', $likeId ) && get_post_type( $likeId ) == 'like' ) {
		...
		wp_die( 'You do not have permission to delete that.' );
	}
}
